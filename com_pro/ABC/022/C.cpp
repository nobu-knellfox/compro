...
signed main()
{
	...
	REP(k, n)REP(i, n)REP(j, n)md[i+1][j+1] = min(md[i+1][j+1], md[i+1][k+1] + md[k+1][j+1]);
 
	int ans = inf;
 
	REP(i, graph2.size()) {
		REPS(i+1,j, graph2.size()) {
			auto nowi = graph2[i];
			auto nowj = graph2[j];
 
			ans = min(ans, nowi.cost + nowj.cost + md[nowi.to][nowj.to]);
		}
	}
