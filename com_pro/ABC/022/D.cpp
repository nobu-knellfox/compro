...
signed main()
{
	...
	for (auto x : a) {
		double dx = abs(ma.first - x.first);
		double dy = abs(ma.second - x.second);
		maxia = max(maxia, dx*dx + dy*dy);
	}

	double maxib = 0;

	for (auto x : b) {
		double dx = abs(mb.first - x.first);
		double dy = abs(mb.second - x.second);
		maxib = max(maxib, dx*dx + dy*dy);
	}

	cout.precision(10);
	cout << sqrt(maxib) / sqrt(maxia) << endl;
}
